...
~\\
HEADER Harry Albert \\
HEADER\_LEFT (111)111-1111 \\
HEADER\_RIGHT hsa1@williams.edu \\
~\\
EDUCATION\\
...
HEADER\_RIGHT hsa1@williams.edu \\
~\\
GENERIC\_SECTION Languages \& Technologies\\
ITEM Javascript/Typescript, Python, HTML + CSS, F\#, Java, C \\
ITEM React, Scikit-Learn, PyTorch, SolidJS, REST APIs \\
~\\
GENERIC\_SECTION Projects\\
ITEM BOLD "ML Lung Cancer Prediction Tool": Created a neural network to predict lung cancer based on several easily self-diagnosable conditions. Published website where users can interact with this model to democratize access and allow users to play with the model. \\
...
\begin{center}
	\textbf{Example 3:}
\end{center}
This example program introduces the idea of a resume as a work in progress. To facilitate continued work on resumes, CAREER supports comments. These comments allow users to write out future todo lists, planned modifications, or other general notes. \\
~\\
HEADER Harry Albert \\
HEADER\_LEFT (111)111-1111 \\
HEADER\_RIGHT hsa1@williams.edu \\
...
\hspace{\parindent} ITEM BOLD "Booktrak": Independently designed and created Booktrak for WSO (student run website for Williams College). Booktrak is a platform where Williams students can buy and sell used books, and link their book listings to specific classes in order to better facilitate used book sales. \\

\subsection*{Language Concepts}
The core concept of this language is that there are modifiers and text. Text is a primitive, and it is defined as any string that is not a modifier. For example,  in the line "HEADER Harry Albert," "Harry Albert" is the text. Modifiers are (conventionally all caps) functions that change how text is displayed. For example, the GENERIC\_SECTION modifier bolds the text that follows it and adds a horizontal line above it to indicate a new section. Ideally, users will be able to create their own modifiers to make the language more groweable. Finally, modifiers can be stacked. For example, ITEM creates a bullet pointed text, and BOLD (used within an item) creates a bolded piece of text. When quotes are included after a modifier, the modifier will apply to all the text included in the quotes. If quotes are not included, the modifier will apply to all text until a new line.

\subsection*{Formal Syntax}

$<$expression$>$ = $<$line$>$$[<$expression$>]$\\
$<$line$>$ = $<$formatted\_text$>^*$ \textbackslash n\\
...
~\\
$<$string$>$ =  $<$str\_char$>^*$ \\
$<$str\_char$>$ = $<$letter$> | <$number$> | $ \textvisiblespace\\
$<$letter$>$ = $\alpha \in \{a, ..., z\}$ \\
$<$number$>$ = $\alpha \in \{0, ..., 9\}$\\
~\\

\subsection*{Semantics}
CAREER is essentially composed of many lines of strings. Thus, the primitive form in CAREER can be thought of as either a character or a string (a list of characters), depending on how deep you want to go into the program implementation. Basic strings can only be composed of numbers, letters, and spaces (this is a potential shortcoming that could be improved in the future). Modifier function strings (which are used as commands for text formatting) can have any character. However, all of these function strings are defined within the definition of CAREER itself, instead of in an actual CAREER program. Modifier function strings are all uppercase and snake case by convention. \\

The combining forms for modifier strings and basic strings are modifiers, lines, and expressions. Modifiers combine modifying functions and other text (other text could include strings or text that has already had some different modifying function applied to it, or both). The left side of a modifier is a modifier function string such as "HEADER\_LEFT" or "BOLD". The right side of a modifier is either another modifier or a basic string, or both. Modifier bodies extend as far right as possible. This means that, in the example "ITEM BOLD hello world", the BOLD command would apply to the entire string "hello world". If the user wants BOLD to only apply to a part of the string, they can use quotes to limit the scope of the function. Returning to the previous example, this would look like 'ITEM BOLD "hello" world', a command which would result in only "hello" being bolded. The other combining form in CAREER is a line. \\

A line is simply a list of modifiers and strings, and each line corresponds with an actual line from the input and outputs an actual formatted line in the resluting resume. The result of evaluating 'ITEM BOLD "hello" world' would be Line([Modifier ("ITEM", [Modifier ("BOLD", [String "hello"]); String " world"])]). Notice that the modifier "BOLD" only applies to the string "hello", but the modifier "ITEM" applies to all parts of the string. Finally, an expression is simply a list of lines. Each AST will only contain one expression which contains a list of all of the lines in a resume. \\
