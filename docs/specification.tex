...
~\\
*HEADER Harry Albert \\
*SUB\_HEADER (111)111-1111 \\
*SUB\_HEADER hsa1@williams.edu \\
~\\
*EDUCATION\\
...
$<$line$>$ = *$<$item\_mod$><$formatted\_text$>^*$ \textbackslash n $| <$formatted\_text$>^*$ \textbackslash n\\
...
The combining forms for modifier strings and basic strings are modifiers, lines, and expressions. Modifiers combine modifying functions and other text (other text could include strings or text that has already had some different modifying function applied to it, or both). The left side of a modifier is a modifier function string such as "*SUB\_HEADER" or "*BOLD". The right side of a modifier is either another modifier or a basic string, or both. Modifier bodies extend as far right as possible. This means that, in the example "*ITEM *BOLD hello world", the BOLD command would apply to the entire string "hello world". If the user wants BOLD to only apply to a part of the string, they can use quotes to limit the scope of the function. Returning to the previous example, this would look like '*ITEM *BOLD "hello" world', a command which would result in only "hello" being bolded. The other combining form in CAREER is a line. \\
